 ...
 * 构造认证与设备请求头
 *
 * 设备签名 = sha256(deviceId + timestamp)，与后端 DeviceToken 中间件一致。
 * SSE 连接（composables/sse/useSse.ts）也用它——签名逻辑只此一份，改这里即可。
 */
...
http.interceptors.request.use(
  async (config: InternalAxiosRequestConfig) => {
    config.headers = Object.assign({}, config.headers, await buildRequestHeaders());
    return config;
  },
...
/** 会话结束：清理登录态并跳转登录 */
async function endSession(text: string): Promise<never> {
  await redirectToLogin(text);
  ...
  switch (body.code) {
    ...
    case ApiCodeEnum.LOGIN_EXPIRED: {
      ...
      try {
        ...
        if (token) {
          config.headers.set(HeaderAuthorization, `Bearer ${token}`);
        }
        return http(config);
      } catch {
        return endSession(LoginExpiredText);
